...
describe("Integration: Complete User Flows", () => {
  ...
  describe("Scenerio 4: Authorization and Security", () => {
    let owner: User;
    let otherUser: User;
    let space: Space;
    ...
    beforeEach(async () => {
      ...
      const spaceResponse = await authenticatedRequest(
        ...
        "/api/v1/spaces",
        owner.id,
        { name: "Owner Space", isPublic: true },
      );

      space = spaceResponse.json().data;
    });

    it("should enforce space ownership rules", async () => {
      // other user tries to update space
      ...
      expect(updateResponse.statusCode).toBe(403); // unauthorized to update FORBIDDEN

      // other user tries to delete space
      ...
      // owners can update
      const ownerUpdateResponse = await authenticatedRequest(
        "PATCH",
        `/api/v1/spaces/${space.id}`,
        owner.id,
        { name: "Updated Space" },
    ...
    it("should allow space owner to delete any song", async () => {
      // other user add song
      const songResonse = await authenticatedRequest(
        "POST",
        "/api/v1/songs",
        otherUser.id,
        {
          spaceId: space.id,
          youtubeUrl: "https://www.youtube.com/watch?v=bla0YBsd0DQ",
        },
      );
      const song = songResonse.json().data;
      // owner deletes the song
      const ownerDelete = await authenticatedRequest(
        "DELETE",
    ...
    it("should prevent unauthorized song delete", async () => {
      // user A adds a song
      ...
      const songA = songResponse.json().data;

      // user B tries to delete songs
      const userB = await createTestUser({ email: "userb@example.com" });
      const deleteResponse = await authenticatedRequest(
        "DELETE",
        `/api/v1/songs/${songA.id}`,
        userB.id,
      );
      expect(deleteResponse.statusCode).toBe(403);
    });
  ...
  describe("Scenerio 5: Edge cases and Error Handling", () => {
    let user: User;
    let space: Space;
    ...
    beforeEach(async () => {
      ...
      const spaceResponse = await authenticatedRequest(
        ...
        "/api/v1/spaces",
        user.id,
        { name: "Test Space", isPublic: true },
      );
      space = spaceResponse.json().data;
    });

    it("should prevent duplicate song in queue", async () => {
      ...
      expect(songResponse.statusCode).toBe(201);

      // add same song again
      const againResponse = await authenticatedRequest(
        "POST",
        "/api/v1/songs",
        user.id,
        {
      ...
      expect(againResponse.statusCode).toBe(409);
      expect(againResponse.json().message).toContain(
        "This song is already in queue",
      );
    });

    it("should handle non-existing resource", async () => {
      // try to get update or delete non existing space
      const fakeId = crypto.randomUUID();
      const spaceResponse = await testApp.inject({
        method: "GET",
        url: `/api/v1/spaces/${fakeId}`,
      });
      expect(spaceResponse.statusCode).toBe(404);

      // try to vote a non existing song
      const voteResponse = await authenticatedRequest(
        "POST",
        `/api/v1/votes`,
        user.id,
        {
      ...
      // try to delete or update non exisiting song
    });

    it("should validate input data", async () => {
      // space name length
      const longName = await authenticatedRequest(
        "POST",
        "/api/v1/spaces",
        user.id,
        { name: "a".repeat(200), isPublic: true },
      );
      ...
      const songResponse = await authenticatedRequest(
        ...
        {
          spaceId: space.id,
          youtubeUrl: "https://www.youtube.com/watch?v=6POZlJAZsok",
        },
      );
      const song = songResponse.json().data;

      const invalidSong = await authenticatedRequest(
        "POST",
        "/api/v1/votes",
        user.id,
        {
      ...
      expect(invalidSong.statusCode).toBe(400);
    });

    // also test pagination
  });

  describe("Scenerio 6: Concurrent Operations", () => {
    it("should handle simultaneous votes correctly", async () => {
      ...
      // first user creates space and add song
      const spaceResponse = await authenticatedRequest(
        "POST",
        "/api/v1/spaces",
        users[0].id,
        { name: "Concurrent Space", isPublic: true },
      );

      const space = spaceResponse.json().data;
      ...
      const songResponse = await authenticatedRequest(
        ...
        {
          spaceId: space.id,
          youtubeUrl: "https://www.youtube.com/watch?v=eftlnpWI-lc",
        },
      );
      const song = songResponse.json().data;

      // all users vote simultaneouly
      const votePromises = users.map((user) => {
        return authenticatedRequest("POST", "/api/v1/votes", user.id, {
          songId: song.id,
          value: 1,
        });
      });
      ...
      // check all votes succeeded
      voteResponses.forEach((r) => {
      ...
      const voteDetails = voteDetailsResponse.json().data;
      expect(voteDetails.score).toBe(5);
      expect(voteDetails.voteCount.total).toBe(5);
      ...
      expect(dbSong?.votes.length).toBe(5);
    });

    it("should handle race condition when adding duplicate songs", async () => {
      const user = await createTestUser();
      const spaceResponse = await authenticatedRequest(
        "POST",
        "/api/v1/spaces",
        user.id,
        { name: "Race Condition Space", isPublic: true },
      );

      const space = spaceResponse.json().data;
      const youtubeUrl = "https://www.youtube.com/watch?v=V9PVRfjEBTI";

      const [res1, res2] = await Promise.all([
        authenticatedRequest("POST", "/api/v1/songs", user.id, {
          spaceId: space.id,
          youtubeUrl,
        }),
        authenticatedRequest("POST", "/api/v1/songs", user.id, {
      ...
      console.log("Response 1:", res1.statusCode, res1.json());
      console.log("Response 2:", res2.statusCode, res2.json());
